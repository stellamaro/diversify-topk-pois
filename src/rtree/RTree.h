...
protected:
  ...
  void PrintNode(const Node* a_node) const;
...
RTREE_TEMPLATE
typename RTREE_QUAL::Node* RTREE_QUAL::GetRoot() const
{
    return m_root;
}

RTREE_TEMPLATE
void RTREE_QUAL::Print() const
{
    Node* first = m_root;
    PrintNode(first);
}
...
RTREE_TEMPLATE
void RTREE_QUAL::PrintNode(const Node* a_node) const {

    if(!(a_node->IsLeaf()))
    {
        for(int i = 0; i < a_node->m_count; ++i)
        {
            PrintNode(a_node->m_branch[i].m_child);
        }
        if (a_node) {
